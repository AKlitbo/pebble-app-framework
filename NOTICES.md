...
- **What Ships:** `doxygen-awesome.css`, `doxygen-awesome-darkmode-toggle.js` and `doxygen-awesome-interactive-toc.js`, copied in unchanged. The framework's own look sits on top of them in `docs/doxygen/pebble.css`, so moving to a newer release means copying the three files over again. The toggle's two icons come from Google's Material icons under the Apache License 2.0, as that file notes
