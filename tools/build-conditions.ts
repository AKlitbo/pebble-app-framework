 ...
 *
 * Reads the engine's ts/weather/conditions.ts and ts/pkjs/wire.ts, the two tables the phone owns,
 * and writes
...
function bannerLines(fileName: string, brief: string, source = 'ts/weather/conditions.ts', group = 'lib_ui') {
  return [
    ...
    ` * @brief ${brief}`,
    ' *',
    ` * Generated from the engine's \`${source}\`, so the phone and the watch read the same numbers.`,
    ' *',
    ` * @ingroup ${group}`,
    ' */',
    '#pragma once',
 ...
 *
 * @param caps The cap table from the engine's wire module.
