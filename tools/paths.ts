...
/**
 * Whether the engine is mounted at <workspace>/lib/ beside a watchfaces/ folder. Both have to hold,
 * so a standalone checkout never mistakes whatever folder it sits in for a repo of faces.
 */
export const MOUNTED = path.basename(ENGINE) === 'lib' && fs.existsSync(path.join(ENGINE, '..', 'watchfaces'));
